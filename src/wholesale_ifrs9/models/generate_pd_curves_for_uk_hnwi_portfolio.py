...
class Main(trac.TracModel):
    ...
    def define_inputs(self) -> tp.Dict[str, trac.ModelInputSchema]:
        ...
        return {
            "hnwi_rating_master_scale": trac.ModelInputSchema(rating_master_scale_schema),
            "hnwi_transition_matrix": trac.ModelInputSchema(transition_matrix_schema),
    ...
    def define_outputs(self) -> tp.Dict[str, trac.ModelOutputSchema]:
        ...
        return {
            "uk_hnwi_pd_term_structures": trac.define_output_table(uk_corporate_pd_term_structures_schema.table.fields, label="UK High Net Worth Individuals PD term structures"),
        }
    ...
    def run_model(self, ctx: trac.TracContext):
        ...
        hnwi_transition_matrix = ctx.get_pandas_table("hnwi_transition_matrix")
        ...
        #############################################################################################
        #                   UK High Net Worth Individuals portfolio calculation                     #
        #############################################################################################

        logger.info(f"Generating the High Net Worth Individuals PD curves for a {reporting_date} reporting date")

        tm = TransitionMatrix(matrix=hnwi_transition_matrix, reporting_date=str(reporting_date), years=ceil(total_months / 12))
        __external_ratings = tm.ratings.tolist().remove("Default")

        hnwi_pd_term_structures = _calculate_PiT_PDs(
            tm=tm.interpolated_term_structures,
            mrs=hnwi_rating_master_scale,
            external_ratings=tm.ratings.tolist(),
            internal_ratings=hnwi_rating_master_scale['Rating'].tolist(),
            scenario_Names=scenarios.scenario_Names,
            macroImpacts=macroImpacts
        )

        hnwi_pd_term_structures = calculate_weighted_average_curves(
            pd_term_structures=hnwi_pd_term_structures,
            masterRatingScale=hnwi_rating_master_scale,
            total_months=total_months
        )

        # Output the datasets
        ctx.put_pandas_table("uk_hnwi_pd_term_structures", hnwi_pd_term_structures)


if __name__ == "__main__":
    import tracdap.rt.launch as launch

    launch.launch_model(Main, "wholesale_ifrs9/config/generate_pd_curves_for_uk_hnwi_portfolio.yaml", "wholesale_ifrs9/config/sys_config.yaml")
