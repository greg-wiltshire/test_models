...
class Main(trac.TracModel):
    ...
    def define_inputs(self) -> tp.Dict[str, trac.ModelInputSchema]:
        ...
        retail_transition_matrix_schema = trac.load_schema(schemas, "retail_transition_matrix_schema.csv")
        ...
        return {
            ...
            "retail_transition_matrix": trac.ModelInputSchema(retail_transition_matrix_schema),
    ...
    def define_outputs(self) -> tp.Dict[str, trac.ModelOutputSchema]:
        ...
        return {
            "gcc_retail_pd_term_structures": trac.define_output_table(gcc_retail_pd_term_structures_schema.table.fields, label="GCC retail PD term structures"),
            }
    ...
    def run_model(self, ctx: trac.TracContext):
        ...
        retail_transition_matrix = ctx.get_pandas_table("retail_transition_matrix")
        ...
        #############################################################################################
        #                               GCC Retail portfolio calculation                            #
        #############################################################################################

        logger.info(f"Generating the Retail PD curves for a {reporting_date} reporting date")

        tm = TransitionMatrix(matrix=retail_transition_matrix, reporting_date=str(reporting_date), years=ceil(total_months / 12))

        external_ratings = tm.ratings.tolist()
        if "Default" in external_ratings:
            external_ratings.remove("Default")

        retail_pd_term_structures = _calculate_PiT_PDs(
            tm=tm.interpolated_term_structures,
            mrs=retail_rating_master_scale,
            external_ratings=external_ratings,
            internal_ratings=retail_rating_master_scale['Rating'].tolist(),
            scenario_Names=scenarios.scenario_Names,
            macroImpacts=macroImpacts
        )

        retail_pd_term_structures = calculate_weighted_average_curves(
            pd_term_structures=retail_pd_term_structures,
            masterRatingScale=retail_rating_master_scale,
            total_months=total_months
        )

        # Output the datasets
        ctx.put_pandas_table("gcc_retail_pd_term_structures", retail_pd_term_structures)


if __name__ == "__main__":
    import tracdap.rt.launch as launch

    launch.launch_model(Main, "wholesale_ifrs9/config/generate_pd_curves_for_gcc_retail_portfolio.yaml", "wholesale_ifrs9/config/sys_config.yaml")
